    ...
    while True:
        ...
        if event == "Aplicar":
            ...
            if len(selected_date) == 0 and len(selected_modo) == 0 and len(selected_name) == 0:
                sg.popup_error("Selecione um filtro válido!")
            ...
            elif len(selected_name) != 0 and len(selected_date) != 0 and len(selected_modo) != 0:   #Todos estão selecionados
                for item in view:
                    item[4] = util.format_date(item[4])
                    if len(selected_date_final) != 0:
            ...
            elif len(selected_date) != 0 and len(selected_modo) == 0 and len(selected_name) == 0:   #Data selecionada e modalidade não
                for item in view:
                    item[4] = util.format_date(item[4])
                    if len(selected_date_final) != 0:
                        # Comparando a data
                        ...
                        if item[4] == selected_date:  # Comparando a data
                            ...
                            item[1] = util.coin_format(str(item[1]))
                            search_list.append(item)
            elif len(selected_date) == 0 and len(selected_modo) != 0 and len(selected_name) == 0:   #Modalidade selecionada e data não
                for item in view:
                    item[4] = util.format_date(item[4])
